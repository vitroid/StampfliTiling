                        ...
                        for l in edges[i]:
                            if k < l:
                                if (edges[j].has_key(k) and
                                    ...
                                    tets[(i,j,k,l)] = wrap((atoms[i][0]+dx,
                                                            atoms[i][1]+dy,
                                                            atoms[i][2]+dz))
    return tets
...
#print len(tets)
#for tet in tets:
#oval(atoms[52][0]-10,atoms[52][1]-10,20,20)
for tet in tets:
    c = tets[tet]
    fill(0,0,c[2]/box[2])
    oval(c[0]-5,c[1]-5,10,10)
    fill(0)
print len(atoms),len(tets)
tetc = tets.values()
tetedges = bond3d(tetc,thres/2)
for i in tetedges:
    for j in tetedges[i]:
        line(tetc[i][0],tetc[i][1],tetc[i][0]+tetedges[i][j][0],tetc[i][1]+tetedges[i][j][1])
        print sqrt(tetedges[i][j][0]**2 + tetedges[i][j][1]**2 + tetedges[i][j][2]**2)
